...
class Causality(IntFlag):
    ...
    INDEPENDENT = 1 << 5
    ANY = (
        INPUT | OUTPUT | PARAMETER | CALCULATED_PARAMETER | LOCAL | INDEPENDENT
    )
...
class TwinRuntime:
    ...
    def twin_is_cross_platform(file_path):
        ...
        with zipfile.ZipFile(file_path) as zip_handler:
            ...
            has_windows = any(
                [name.startswith("binaries/win64/") for name in zip_contents]
            )
            has_linux = any(
                [name.startswith("binaries/linux64/") for name in zip_contents]
            )
    ...
    def get_model_name(file_path: str) -> str:
        ...
        if file_path.suffix in [".fmu"]:
            ...
            raise TwinRuntimeError(
                "Cannot read encrypted description XML files "
                "from .tbrom or .twin models"
            )
        else:
            raise TwinRuntimeError(
                "Unsupported file extension: " f"{file_path.suffix}"
    ...
    def __init__(
        ...
        self._TwinOpenWithFmiType = (
            self._twin_runtime_library.TwinOpenWithFmiType
        )
        ...
        self._TwinGetVariableNames = (
            self._twin_runtime_library.TwinGetVariableNames
        )
    ...
    def twin_get_number_variables(
        ...
        self.evaluate_twin_status(
            self._twin_status, self, "twin_get_number_variables"
        )
    ...
    def twin_get_variable_names(
        ...
        self.evaluate_twin_status(
            self._twin_status, self, "twin_get_variable_names"
        )
    ...
    def twin_get_rom_output_basis(self, model_name):
        ...
        self.evaluate_twin_status(
            self._twin_status, self, "twin_get_rom_output_basis"
        )
    ...
    def twin_get_rom_input_basis(self, model_name, field_name):
        ...
        self.evaluate_twin_status(
            self._twin_status, self, "twin_get_rom_input_basis"
        )
