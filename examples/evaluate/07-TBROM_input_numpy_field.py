...
inputfieldsnapshot = download_file(
    "TEMP_1.bin", "twin_input_files/inputFieldSnapshots", force_download=True
    )
...
# separate program).
temperature_array = np.fromfile(
    inputfieldsnapshot, dtype=np.double, offset=8
    ).reshape(-1,)
...
outfield_bolts = twin_model.generate_snapshot(
    romname, on_disk=False, named_selection=out_ns
    )  
...
points_bolts = twin_model.generate_points(
    romname, on_disk=False, named_selection=out_ns
    )
################################################################################
