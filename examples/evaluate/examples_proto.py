...
ds = dpf.DataSources()
ds.set_result_file_path(cfd_file)
...
mesh_data = twin_model._tbroms[rom_name]._meshdata
...
#outfield = twin_model.generate_snapshot(rom_name, False)

#outfield = outfield.reshape(-1, 3)
#print(max(np.sqrt((outfield * outfield).sum(axis=1))))
#print(max(outfield[:,0]))
#print(max(outfield[:,1]))
#print(max(outfield[:,2]))
#outfield = twin_model.generate_snapshot(rom_name, True, 'shell')
#outfield = twin_model.generate_points(rom_name, True, 'shell')
#plotter = pv.Plotter()
#plotter.add_mesh(mesh_data)
#plotter.show()
